...
void test_llist_deque(void) {
  float *f = malloc(sizeof(float));
  *f = 2.5f;
  llist_push_back(list, f);
  CU_ASSERT(f == list->anchor->prev->data);
  CU_ASSERT(*f == *((float *)list->anchor->prev->data));
  CU_ASSERT(f == llist_peek_back(list));
  CU_ASSERT(f == llist_pop_back(list));
  CU_ASSERT(list->size == 0);
  free(f);
}
